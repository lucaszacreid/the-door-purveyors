...
function LeadDetail({ label, value, href }: { label: string; value: string; href?: string }) {
  return (
    <div>
      ...
      {href ? (
        <a href={href} className="text-sm text-[#b07d3a] hover:text-[#9a6d31] transition-colors break-all">
          {value}
        </a>
      ) : (
...
export default function AdminPage() {
  ...
  if (!initialized) {
    return (
      <div className="min-h-screen bg-[#1e2d3d] flex items-center justify-center">
        <div className="w-6 h-6 border-2 border-[#b07d3a] border-t-transparent rounded-full animate-spin" />
      </div>
    )
  }
  ...
  if (!isLoggedIn) {
    return (
      <div className="min-h-screen bg-[#1e2d3d] flex items-center justify-center px-6">
        <div className="w-full max-w-sm">
          <div className="mb-10 text-center">
            <div className="w-10 h-px bg-[#b07d3a] mx-auto mb-6" />
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <input
              ...
              className="w-full bg-white/5 border border-white/10 text-white px-4 py-3 text-sm font-body placeholder:text-white/20 focus:outline-none focus:border-[#b07d3a] transition-colors"
            />
            {loginError && <p className="text-red-400 text-sm font-body">{loginError}</p>}
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-[#b07d3a] hover:bg-[#9a6d31] text-black font-semibold py-3 text-sm tracking-wider uppercase transition-colors disabled:opacity-50 font-body"
            >
  ...
  return (
    <div className="min-h-screen bg-[#1e2d3d] font-body">
      {/* Header */}
      ...
      <main className="max-w-7xl mx-auto px-6 py-10">
        <div className="flex items-center justify-between mb-8">
          ...
          <button
            onClick={() => fetchLeads()}
            className="text-[#b07d3a] hover:text-[#9a6d31] text-sm transition-colors flex items-center gap-2"
          >
                    ...
                    <>
                      ...
                      >
                        ...
                        <td className="py-4 pr-6">
                          <span className="text-[#b07d3a] text-[10px] font-semibold px-2.5 py-1 border border-[#b07d3a]/30 uppercase tracking-wider whitespace-nowrap">
                            {lead.doorType}
                          </span>
                        </td>
                      ...
                      {isExpanded && (
                        <tr key={`${lead.id}-expanded`} className="border-b border-white/5">
                          <td colSpan={7} className="px-0 py-0">
                            <div className="bg-white/[0.03] border-l-2 border-[#b07d3a] mx-0 px-8 py-6">
                              ...
                              <div className="flex items-center gap-4 mt-6 pt-5 border-t border-white/5">
                                <a
                                  href={`mailto:${lead.email}`}
                                  className="inline-flex items-center gap-2 bg-[#b07d3a] hover:bg-[#9a6d31] text-black text-xs font-semibold px-4 py-2 tracking-wider uppercase transition-colors"
